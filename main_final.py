...
class Color():
    def __init__(self, imgsize=256, batchsize=4):
        ...
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake_logits,labels= tf.ones_like(disc_fake_logits))) + self.l1_scaling * tf.reduce_mean(tf.abs(self.real_images - self.generated_images))
    ...
    def discriminator(self, image, y=None, reuse=False):
        ...
        #with tf.variable_scope("descriminator".format(i)):
                h0 = lrelu(conv2d(image, self.df_dim, name='d_h0_conv')) # h0 is (128 x 128 x self.df_dim)
                h1 = lrelu(self.d_bn1(conv2d(h0, self.df_dim*2, name='d_h1_conv'))) # h1 is (64 x 64 x self.df_dim*2)
                h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim*4, name='d_h2_conv'))) # h2 is (32 x 32 x self.df_dim*4)
                h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim*8, d_h=1, d_w=1, name='d_h3_conv'))) # h3 is (16 x 16 x self.df_dim*8)
    ...
    def sample(self):
        ...
        data = glob(os.path.join("images_input", "*.jpg"))
        ...
        for i in range(min(100, int(datalen/self.batch_size) )):
            ...
            ims("results/"+str(i)+"_shaded.jpg",merge_color(recreation, [self.batch_size_sqrt, self.batch_size_sqrt]))
            ims("results/"+str(i)+"_origin.jpg",merge_color(batch_normalized, [self.batch_size_sqrt, self.batch_size_sqrt]))
            ims("results/"+str(i)+"_sketch.jpg",merge_color(batch_edge, [self.batch_size_sqrt, self.batch_size_sqrt]))
            ims("results/"+str(i)+"_colorblock.jpg",merge_color(batch_colors, [self.batch_size_sqrt, self.batch_size_sqrt]))
