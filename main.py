...
class Color():
    def __init__(self, imgsize=256, batchsize=4):
        ...
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(disc_fake_logits, tf.ones_like(disc_fake_logits))) + self.l1_scaling * tf.reduce_mean(tf.abs(self.real_images - self.generated_images))
